...
class HydraEntryPoint:
    ...
    def _get_module(self) -> tp.Any:
        ...
        if benchpath not in sys.path:
            ...
            # add controllable_navi, for legacy buffers
            sys.path.append(str(self._script_path.parent))
        already_imported = any("controllable_navi" in x for x in sys.modules)
        module = importlib.import_module("controllable_navi." + self._script_path.stem)
        ...
        if module.__file__ is None or not module.__file__.startswith(benchpath):
            if already_imported:
                logger.warning(
                    "controllable_navi had already been imported, using {module.__file__}"
                )
            else:
                raise RuntimeError(
    ...
    def workspace(self, cfg) -> tp.Any:
        return self._get_module().Workspace(cfg)
...
class CopiedBenchmark(HydraEntryPoint):
    def __init__(self, folder: PathLike, name: str) -> None:
        ...
        if self.code.exists():
            ...
            shutil.copytree(
                Path(__file__).parents[1] / "controllable_navi",
                self.code / "controllable_navi",
                ignore=shutil.ignore_patterns("exp_*"),
            )
        super().__init__(self.code / "controllable_navi" / f"{name}.py")
