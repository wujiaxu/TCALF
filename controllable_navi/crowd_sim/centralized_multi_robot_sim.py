...
# from controllable_navi.crowd_sim.crowd_sim import n_laser,laser_angle_resolute,laser_min_range,laser_max_range #merged to cfg
...
class CentralizedMultiRobotWorld(dm_env.Environment):
    ...
    def random_fix_agent_num(self,agent_num):
        assert agent_num>=self._robot_num[0]
        assert agent_num<self._robot_num[1]+1
        self.current_robot_num = agent_num #np.random.randint(self._robot_num[0],self._robot_num[1]+1)
    ...
    def reset(self,test_case=None):
        ...
        if self.case_counter[self.phase]%self._regen_map_every==0:
            self.randomize_map()
        if self._robot_num[1]==2 and test_case is None: #TOY ENV
